...
def generate_samples_yaml_from_tsv(
    input_tsv: str,
    output_yaml: Optional[str] = None,
) -> str:
    """Create a samples YAML file from a two-column TSV file.

    The TSV file should contain ``sample_name<TAB>path`` per line. Lines that are
    empty or begin with ``#`` are ignored.

    Args:
        input_tsv: Path to the two-column TSV file.
        output_yaml: Optional name for the output YAML file. If not provided a
            timestamped filename will be used.
    ...
    Raises:
        ...
        ValueError: If no valid sample/path pairs are found.
    """
    ...
    if not pairs:
        raise ValueError("No valid sample/path pairs found in TSV file")

    samples_dict: Dict[str, Dict[str, str]] = {"samples": {}}
    for name, path in pairs:
        samples_dict["samples"][name] = path

    if output_yaml is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_yaml = f"samples_{timestamp}.yaml"
    ...
    with open(output_path, "w") as handle:
        yaml.dump(samples_dict, handle, default_flow_style=False, indent=2)
